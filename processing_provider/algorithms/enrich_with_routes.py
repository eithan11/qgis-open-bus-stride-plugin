...
class EnrichWithRoutes(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        line_ref_field = self.parameterAsString(parameters, self.LINE_REF_FIELD, context)
        
        if input_layer is None:
            raise QgsProcessingException(self.tr('Invalid input layer'))
        
        # Step 1: Extract unique line_refs and date range from data
        feedback.pushInfo(self.tr('Step 1/3: Extracting unique line references and date range...'))
        ...
        feedback.pushInfo(
            self.tr(f'Date range: {date_from} to {date_to}')
        )
            ...
            if value is not None and value != NULL:
                try:
                    ...
                    feedback.reportError(
                        self.tr(f'Invalid line_ref value: {value}')
                    )
    ...
    def _fetch_route_data(self, line_refs, date_from, date_to, feedback):
        ...
        try:
            ...
            if data and len(data) > 0:
                ...
                for route in data:
                    ...
                    if line_ref is not None:
                        ...
                        if line_ref not in route_data_map:
                            ...
                            feedback.pushInfo(
                                self.tr(f'  Line {line_ref}: {route.get("route_long_name", "N/A")}')
                            )
                ...
                feedback.pushInfo(
                    self.tr('No route data received from API')
                )
                # Mark all as None
                for line_ref in line_refs:
                    route_data_map[line_ref] = None
            ...
            feedback.reportError(
                self.tr(f'Error fetching route data: {str(e)}')
            )
    ...
    def _create_output_sink(self, parameters, context, input_layer):
        ...
        (sink, dest_id) = self.parameterAsSink(
            ...
            input_layer.wkbType(), input_layer.sourceCrs()
        )
    ...
    def _enrich_features(self, input_layer, line_ref_field, route_data_map, 
                         sink_info, feedback):
        """
